...
const CONTAINER_CLASS =
  "relative w-full max-w-[min(26rem,92vw)] sm:max-w-[min(28rem,94vw)] md:max-w-[30rem]";
    ...
    <div className="h-screen min-h-[100dvh] overflow-y-auto overflow-x-hidden bg-background">
      ...
      <div className={CONTAINER_CLASS}>
        ...
        >
          ...
          {/* Logo + branding */}
          <div className="flex flex-col items-center text-center">
            <div className="flex h-14 w-14 sm:h-20 sm:w-20 md:h-24 md:w-24 items-center justify-center rounded-xl sm:rounded-2xl bg-gradient-to-br from-primary to-accent shadow-lg shadow-primary/20 ring-2 ring-white/10">
              <Play className="h-7 w-7 sm:h-9 sm:w-9 md:h-10 md:w-10 text-background shrink-0" fill="currentColor" />
            </div>
            <h1 className="mt-3 sm:mt-4 bg-gradient-to-r from-primary to-accent bg-clip-text text-2xl font-semibold tracking-tight text-transparent sm:text-3xl md:text-4xl">
              {t("common.appName")}
            </h1>
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.06, duration: 0.25, ease: [0.25, 0.46, 0.45, 0.94] }}
            className="mt-6 w-full sm:mt-8"
          >
            <GlassCard padding="lg" className="rounded-2xl shadow-xl sm:p-7 md:p-8">
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                ...
                <div className="flex flex-wrap items-end justify-between gap-3 border-t border-white/10 pt-4 sm:gap-4 sm:pt-5">
                  <div>
                    <span className="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-text-tertiary">
                    ...
                    <div className="flex gap-1 sm:gap-1.5">
                      {THEME_IDS.map((id) => (
                        <button
                          ...
                          type="button"
                          onClick={() => setTheme(id)}
                          className={`rounded-lg px-2 py-1 text-xs font-medium transition-colors sm:px-2.5 sm:py-1.5 ${
                            theme === id
                  ...
                  <div className="min-w-0 flex-1 basis-24 sm:basis-28">
                    <span className="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-text-tertiary">
                    ...
                    <DropdownMenu.Root>
                      <DropdownMenu.Trigger asChild>
                        <button
                          type="button"
                          className="flex w-full items-center justify-between gap-2 rounded-lg border border-divider bg-surface-elevated px-2.5 py-1.5 text-left text-sm text-text-primary transition-colors hover:bg-text-primary/5 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary sm:px-3"
                        >
