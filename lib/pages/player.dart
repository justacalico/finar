...
/// Overlay that runs the sleep timer and shows remaining time; stops playback when timer ends.
class _SleepTimerOverlay extends ConsumerStatefulWidget {
  const _SleepTimerOverlay();

  @override
  ConsumerState<_SleepTimerOverlay> createState() => _SleepTimerOverlayState();
}

class _SleepTimerOverlayState extends ConsumerState<_SleepTimerOverlay> {
  Timer? _timer;
  ...
  void _syncSleepTimer() {
    ...
    if (settings.sleepTimerMinutes == -1) {
      ...
      _sleepEndTime = DateTime.now().add(Duration(minutes: settings.sleepTimerMinutes));
    }
