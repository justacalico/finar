...
for i in {1..30}; do
  ...
  RUN_ID=$(gh api "repos/justacalico/finar/actions/runs?branch=$REF&event=workflow_dispatch&per_page=1" -q '.workflow_runs[0].id' 2>/dev/null || true)
...
for attempt in 1 2 3; do
  ...
  CONCLUSION=$(gh api "repos/justacalico/finar/actions/runs/$RUN_ID" -q '.conclusion' 2>/dev/null || true)
  case "$CONCLUSION" in
    success)
      break
      ;;
