...
PROJECT_PATH="${CI_PROJECT_PATH:-Openlyst/finar}"
MARKER="<!-- mr-pipeline-${PIPELINE_ID} -->"
PIPELINE_URL="https://gitlab.com/${PROJECT_PATH}/-/pipelines/${PIPELINE_ID}"
RUN_URL="https://github.com/justacalico/finar/actions/runs/${RUN_ID}"
...
GitHub run: [View on GitHub](${RUN_URL})"
  ...
  finish)
    ...
    if [ -n "$RUN_ID" ] && command -v gh >/dev/null 2>&1; then
      jobs=$(gh run view "$RUN_ID" -R justacalico/finar --json jobs 2>/dev/null | jq -r '.jobs[] | select(.conclusion != null or .status != null) | "- **\(.name)**: \(.conclusion // .status)"' || true)
      downloads=$(gh api "repos/justacalico/finar/actions/runs/${RUN_ID}/artifacts?per_page=100" 2>/dev/null \
        | jq -r --arg run "$RUN_ID" '.artifacts[] | select(.expired != true) | "- [\(.name)](https://github.com/justacalico/finar/actions/runs/\($run)/artifacts/\(.id))"' || true)
    fi
